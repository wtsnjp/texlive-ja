...
\item[システム用ルート] 複数の\TL を配置できます
  ...
  \begin{description}
  \item[\dir{2024}] 前年のリリース
  \item[\dir{2025}] 最新のリリース
    \begin{description}
    \item[\dir{bin}] ~
      \begin{description}
...
\item[ユーザのホームディレクトリ] （\dir{$HOME}または\winenv{USERPROFILE}）
  \begin{description}
  \item[\dir{.texlive2024}] 前年リリースに適用するユーザごとのデータ（設定や生成物）
  \item[\dir{.texlive2025}] 最新リリースに適用するユーザごとのデータ（設定や生成物）
    \begin{description}
...
で説明しました。デフォルトのインストール先はUnix系では\dir{/usr/local/texlive/2025}、
Windowsでは\winpath{\winenv{SystemDrive}\\texlive\\2025}です。この配置は複数の\TL
...
Windowsでは、通常\winpath{C:\\texlive\\2025}（より一般的に言えば\winpath{%
\winenv{SystemDrive}\\texlive\\2025}）を作成するのに管理者権限は必要ありません。
...
簡単に指定することができます（例：\dir{~/texlive/2025}）。
...
代わりに、通常のインストールを行うと\file{.../2025/texmf.cnf}が生成されます。
どうしてもデフォルト設定を変更する必要が生じた場合は（通常は必要ありません）、この
ファイルに変更を加えてください。メインの設定ファイルは\file{.../2025/texmf-dist/%
