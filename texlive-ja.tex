...
Windowsでは\code{\%SystemDrive\%{\bs}texlive{\bs}2019}です．この配置は複数の\TL を
同時にインストールすることを可能にします．例えば複数のリリース（典型的には，年度
...
言及しましたが，\code{TEXMFLOCAL}（デフォルトでは\dir{/usr/local/texlive/%
texmf-local}または\verb|%SystemDrive%\texlive\texmf-local|）はシステム全体に適用
したいローカルなフォントやマクロを，\code{TEXMFHOME}（デフォルトでは\dir{$HOME/%
texmf}または\verb|%USERPROFILE%\texmf|）はユーザごとの個人用フォントやマクロを
配置するためのものです．これらのディレクトリは毎年のリリースとは切り離して管理
されることが意図されており，そこに含まれているファイルは自動的に各\TL リリース
から検索できるようになっています．したがって，\code{TEXMFLOCAL}の値をメインの\TL
ツリーとあまりかけ離れた位置に変更することは避けるべきです．さもないと将来の
リリースの際に手動で変更を行う必要が発生する可能性があります．
...
%\begin{enumerate}
%
%\item Make sure that you can run the \cmd{tex} program in the first
%place:
%\begin{alltt}
%> \Ucom{tex -{}-version}
...
%have the correct \dirname{bin} subdirectory in your \code{PATH}.  See
...
%\TeX.)  You do have to be running under X for \cmd{xdvi} to work; if
%you're not, or your \code{DISPLAY} environment variable is set
...
%\end{alltt}
%Neither \cmd{gv} nor \cmd{xpdf} are included in \TL{}, so
...
%  \begin{itemize*}
%  \item \cmd{GNU Emacs} is also available natively under Windows, see
%        \url{http://www.gnu.org/software/emacs/emacs.html}.
%  \item \cmd{Emacs with Auc\TeX} for Windows is available from \CTAN.
%        The AuC\TeX\ home page is \url{http://www.gnu.org/software/auctex}.
%  \item \cmd{SciTE} is available from
%        \url{http://www.scintilla.org/SciTE.html}.
%  \item \cmd{Texmaker} is free software, available from
%        \url{http://www.xm1math.net/texmaker}.
%  \item \cmd{TeXstudio} started out as a fork of \cmd{Texmaker} with
%        additional features; \url{http://texstudio.org/}.
%  \item \cmd{TeXnicCenter} is free software, available from
%        \url{http://www.texniccenter.org} and in the pro\TeX{}t distribution.
%  \item \cmd{TeXworks} is free software, available from
%        \url{http://tug.org/texworks} and installed as part of \TL\ for
%        Windows (only).
%  \item \cmd{Vim} is free software, available from
%        \url{http://www.vim.org}.
%  \item \cmd{WinEdt} is shareware available though
%        \url{http://tug.org/winedt} or \url{http://www.winedt.com}.
%  \item \cmd{WinShell} is available from \url{http://www.winshell.de}.
%  \end{itemize*}
%\end{description}
...
%
%\htmlanchor{tlmgr}
%\section{\cmd{tlmgr}: Managing your installation}
%\label{sec:tlmgr}
%
%\begin{figure}[tb]
...
%nowadays.
%
%\subsection{Current \GUI{} interfaces for \cmd{tlmgr}}
%
%\TL{} contains several \GUI s for
%\prog{tlmgr}. Figure~\ref{fig:tlshell} shows \cmd{tlshell},
...
%
%\subsection{Sample \cmd{tlmgr} command-line invocations}
%
...
%  entry \cmd{bitmap2eps} in their `Open with' right-click
%  menu. Bitmap2eps is a simple script which lets \cmd{sam2p} or
%  \cmd{bmeps} do the real work.
...
%  At first, when you preview files with \cmd{dviout}, it will create
...
%  binaries. These include \cmd{gzip}, \cmd{zip},
%  \cmd{unzip}, and the utilities from the \cmd{poppler} suite
%  (\cmd{pdfinfo}, \cmd{pdffonts}, \ldots); no standalone PDF
...
%\begin{ttdescription}
%\item[\sopt{{}-help] print basic usage summary.
%\item[\sopt{{}-verbose] print detailed progress report.
%\item[\sopt{{}-version] print version information, then exit.
%\end{ttdescription}
%
...
%  \samp{S /a:/b} in \cmd{dvips}'s \file{config.ps}.
...
%for these files is named \code{TEXMFCNF}, but we do not recommend
...
%  \cmd{dvips} configuration files, you have to specify the name as
