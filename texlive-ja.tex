...
\item[システム用ルート] 複数の\TL を配置できます
  ...
  \begin{description}
  \item[\dir{2021}] 前年のリリース
  \item[\dir{2022}] 最新のリリース
    \begin{description}
    \item[\dir{bin}] ~
      \begin{description}
...
\item[ユーザのホームディレクトリ] （\dir{$HOME}または\winenv{USERPROFILE}）
  \begin{description}
  \item[\dir{.texlive2021}] 前年リリースに適用するユーザごとのデータ（設定や生成物）
  \item[\dir{.texlive2022}] 最新リリースに適用するユーザごとのデータ（設定や生成物）
    \begin{description}
...
で説明しました．デフォルトのインストール先はUnix系では\dir{/usr/local/texlive/2022}，
Windowsでは\winpath{\winenv{SystemDrive}\\texlive\\2022}です．この配置は複数の\TL
...
Windowsでは，通常\winpath{C:\\texlive\\2022}（より一般的に言えば\winpath{%
\winenv{SystemDrive}\\texlive\\2022}）を作成するのに管理者権限は必要ありません．
...
簡単に指定することができます（例：\dir{~/texlive/2022}）．
...
代わりに，通常のインストールを行うと\file{.../2022/texmf.cnf}が生成されます．
どうしてもデフォルト設定を変更する必要が生じた場合は（通常は必要ありません），この
ファイルに変更を加えてください．メインの設定ファイルは\file{.../2022/texmf-dist/%
...
%
\begin{itemize}[nosep]
\item \cs{show}の出力をファイルにリレダイレクトする\cs{showstream}プリミティブ
  が追加されました．
...
% Note: Optionally block selected discretionary creation. の訳だが詳細不明
\item 任意でブロック選択discretionaryを作成するようになりました．
\item TrueTypeフォント実装を改善しました．
\item \cs{fontdimen}アロケーションがより効率的になりました．
\item 方向同期ノードが後ろに続くローカルparノードのみからなる段落は無視する
  ようになりました．
\end{itemize}
...
\prog{upmendex}: 実験的にアラビア語・ヘブライ語をサポートしました．全般的に文字分類
と多言語サポートが強化されました．

Kpathsea: \code{kpsewchih -all}の結果において通常（非\sopt{all}）の探索の結果が
一番に出力されるようになりました．

\prog{tlmgr}とインフラ：
%
\begin{itemize}[nosep]
\item \url{mirror.ctan.org}についてデフォルトでHTTPSを使うようになりました．
...
Catalina, Big Sur, Monterey) になりました．\dir{x86\_64-darwinlegacy}フォルダのバイナリ
