...
{\TL} 2024以降では、バイナリとしては\upTeX に統合されており、\pTeX プログラムを
実行した場合には\upTeX の\pTeX 互換モードが起動します。

\item[\upTeX]
\pTeX の派生エンジンで、内部コードがUnicodeに変更されています。基本的には\upTeX は
\pTeX の上位互換となっており、Unicodeのもつ機能が全面的に利用できます。
\end{description}
%
...
\item HarfBuzzの一部API用のLuaバインディング。
...
\item \code{pdf->objcompresslevel}を\code{pdf->compress\_level}とともに保存および
  復元します。
\item \code{texlua}のエラーがトレースバックが表示されます。
...
\item writeノードに、未展開トークンテーブル用の追加の\code{value}フィールド
  （\code{getfield}）が追加されました。
...
\item \cmd{ctwill-proofsor}が同一識別子に対する複数定義をサポートしました。
\end{itemize}
