...
%
\begin{itemize}[nosep]
\item 新しい\cs{kcatcode}によりUnicodeラテン文字と16-bit OFM Level-0の結合文字を
  サポート（\code{modifier}: 20, \code{latin\_ucs}: 14）。
\item 先述のエンジン共通変更。
\end{itemize}
...
%
\begin{itemize}[nosep]
\item \cmd{xbb}実行ファイルは\prog{dvipdfmx}へのリンクではなく、独立にメンテナンス
  されるスクリプトとなりました。
