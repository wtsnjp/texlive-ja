...
\item \cmd{dvips}, \cmd{dvipdfm}, \cmd{pdftex}などによって使用されるフォント
  ...
  に配置されるようになりました．
\item \TeX, \MF, \MP がデフォルトでほとんどの入力文字（32以上）をそのまま
  ファイル（例えば\cs{write}の出力先），ログファイル，ターミナルに出力する
  ようになりました．すなわち\eghostguarded{}\verb|^^|\eghostguarded{}表現への
  変換が\emph{行われないように}なりました．\TL~7では，この変換はシステムの
  ロケール設定に依存していました．今後はロケール設定は\TeX のコアプログラム
  の挙動に影響を与えなくなります．もし何らかの理由により\eghostguarded{}%
  \verb|^^|\eghostguarded{}表現による出力が必要な場合は\file{texmf/web2c/%
  cp8bit.tcx}のファイル名を変更してください（将来的にはもう少し簡単な方法を
  用意する予定です）．
