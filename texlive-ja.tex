...
ました．実行が許可されている（数少ない）プログラムのリストは\filename{texmf.cnf}%
に記載されています．詳細については\TL{} 2010のリリースノート（\ref{sec:2010news}%
節）を参照してください．
...
\code{/usr/local/bin}などのディレクトリ（ただし\TeX 関連コマンドがまだインストール
...
すべての設定がお好みのものになったら，テキストモードでは`I'をタイプ，Perl/Tkの\GUI
モードでは`Install TeX Live'ボタンを押すことによってインストールを実行してください．
インストールが完了したら（他にすることがなければ）\ref{sec:postinstall}節まで
スキップしてください．

\subsection{\prog{install-tl}のコマンドラインオプション}
\label{sec:cmdline}

次を実行すると，\prog{install-tl}のコマンドラインオプションの一覧を確認することが
できます：
%
\begin{alltt}
...
\end{alltt}
%
オプション名を指定するには\code{-}または\code{-{}-}をが利用できます．以下では，
一般的なオプションについてのみご説明します：
%
\begin{ttdescription}
\item[-gui]
...
古いオプションである\code{-gui=perltk}と\code{-gui=wizard}も現在でも利用可能です．
...
\item[-lang \meta{言語コード}]
指定した\meta{言語コード}（通常2文字）にあたる言語を使用します（訳注：日本語を使用
したい場合は\code{-lang ja}とします）．インストーラは自動的に最適な言語で起動します
...
を利用して選択することもできます．詳細は\ref{sec:portable-tl}節をご参照ください．

\item[-profile \meta{ファイル}]
インストールプロフィール\meta{ファイル}を読み込み，非対話的にインストールを実行
します．なおインストーラは常に\code{texlive.profile}を\code{tlpkg}ディレクトリに
書き込みます．このファイルを利用すると，まったく同じ設定でインストールを（例えば
別のシステムでも）再実行することが可能です．別の活用方法として，ご自身で用意した
カスタムプロフィール（生成された\code{texlive.profile}を改変するのが簡単です）を
使用したり，空のファイルを指定してすべてをデフォルト設定でインストールしたりする
こともできます．

\item[-repository \meta{リポジトリ}]
...
ものだけをインストールします．なお\code{tlpkg/texlive.tlpdb}は上書きされるので，
ご自身の責任で保存するようにしてください．また，既にあるパッケージの除去は手動で
行う必要があります．このオプションは起動後にインストーラのインターフェースで有効に
することはできません．
\end{ttdescription}

\subsubsection{\code{-repository}オプション}
\label{sec:location}
...
を指していた場合，その下にある\code{archive}サブディレクトリ内の圧縮ファイルが
...
\TL にサポートされている各プラットフォームには\code{TEXDIR/bin}以下に専用の
...
下では\path{$HOME/.profile}（もしくは\path{.profile}から読み込まれている別の
...
\end{sverbatim}
%
cshやtcshの場合は，編集すべきファイルは典型的には\path{$HOME/.cshrc}で，次の
ような行を追加します：
%
\begin{sverbatim}
...
ここでは2つほど大まかな方針を示しておきます．第一に\path{/etc/manpath.config}%
...
\end{sverbatim}
%
第二に各種の検索パスや環境変数のデフォルト値を設定するためのファイル\path{%
/etc/environment}の内容を確認することです．

各UNIX向けのバイナリディレクトリには，\code{man}という名前の\path{texmf-dist%
/doc/man}を指すシンボリックリンクが作成されています．一部の\code{man}コマンド
（例えばmacOSの標準\code{man}コマンド）は，これを利用して自動的にmanページの
設定を最適化してくれる場合があります．
