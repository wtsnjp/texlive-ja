...
Windowsにおいて，UNIXのホームディレクトリに相当するのは\verb|%USERPROFILE%|ディレクトリ
です．Windows Vista以降のWindowsでは，そのデフォルト値は\path{C:\Users\<username>}です．
\file{texmf.cnf}内および\KPS 全般において，\dir{~}はWindowsとUNIXのいずれにおいても
適切に展開されます．
...
プログラムにとって最も重要なものは\code{HKEY{\ul}CURRENT{\ul}USER}, \code{HKEY{\ul}%
LOCAL{\ul}MACHINE}, \code{HKCU}, \code{HKLM}です．レジストリのキー\code{HKCU}はユーザ
のホームディレクトリ内を参照します（\ref{sec:winhome}節を参照）．\code{HKLM}は通常
Windowsディレクトリ内のサブディレクトリです．
