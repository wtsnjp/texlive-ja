...
ものです。この文書の読者はすでにダウンロード、DVDからの読み込み、もしくは
その他の方法により\TL を入手していることと思います。
...
\TL はDVD（\url{https://tug.org/dvd}参照）やISOイメージ、インターネット経由で
インストールすることができます（\url{https://tug.org/texlive/acquire.html}）。
インターネット・インストーラ自体はとても小さなプログラムで、これを実行すると
必要とされるすべてのファイルをインターネットからダウンロードします。

\TL はUSBディスクなどに実行可能な形でインストール（ポータブルインストール）する
ことも可能です（\ref{sec:portable-tl}節を参照）。インストール方法については後ほど
詳述しますが（\p.\pageref{sec:install}）、ここではクイックスタートとして簡易的に
説明します：
...
リスト\email{texhax@tug.org}などが挙げられます。これらのアーカイブには何年分もの
...
\email{texhax@tug.org}（購読せずに投稿して構いません）を通じて質問することも
...
\item[システム用ルート] 複数の\TL を配置できます
  ...
  \begin{description}
  \item[\dir{2025}] 前年のリリース
  \item[\dir{2026}] 最新のリリース
    \begin{description}
    \item[\dir{bin}] ~
      \begin{description}
...
\item[ユーザのホームディレクトリ] （\dir{$HOME}または\winenv{USERPROFILE}）
  \begin{description}
  \item[\dir{.texlive2025}] 前年リリースに適用するユーザごとのデータ（設定や生成物）
  \item[\dir{.texlive2026}] 最新リリースに適用するユーザごとのデータ（設定や生成物）
    \begin{description}
...
拡張したエンジン（いずれもオリジナル処理系の派生。それらも\TeX 処理系と呼ばれて
います）も収録されています：
%
\begin{description}
...
を追加した処理系です。この\TeX エンジンにはMetaPostがライブラリとして組み込まれて
おり、高速なグラフィック生成が可能です。そして何よりも軽量スクリプト言語Lua（\url{%
...
\item[LuaMeta\TeX]
\LuaTeX の後継エンジンで、\ConTeXt のデフォルトエンジンです。柔軟性を最大化する
ため、フォントの取り扱いやバックエンド出力も含め、ほとんどの機能がLuaで実装されて
います。ウェブサイト：\url{https://ctan.org/pkg/context}、リファレンスマニュアル：
\file{texmf-dist/doc/luametatex/base/luametatex.pdf}。
...
\item[Hi\TeX]
モバイル端末のためにデザインされたHINTという独自フォーマットを出力するエンジンです。
ウェブサイト：\url{https://ctan.org/pkg/hitex}。

\item[$\varepsilon$-(u)\pTeX]
日本語組版をネイティブにサポートする処理系です。オリジナルの\pTeX と、内部Unicode化
によって機能が強化された\upTeX もあります（詳細はこのリストの下のコラムを参照）。
...
現在では、Omegaは\TL に独立のプログラムとしては収録されていません。Alephのみが
提供されています。
...
\pTeX の上位互換となっており、JIS外の漢字が使えます。
\end{description}
%
現在の\TL に収録されている(u)\pTeX は厳密にはオリジナルのものではなく、
$\varepsilon$-拡張のすべてと\pdfTeX 由来の機能の一部が追加されている
$\varepsilon$-(u)\pTeX です。詳細は\file{texmf-dist/doc/ptex/ptex-%
base/eptexdoc.pdf}を参照してください。

また\LuaTeX-jaというマクロパッケージを用いると\LuaTeX でも日本語組版を実現できます。
詳しくはウェブサイト\url{https://github.com/luatexja/luatexja}またはドキュメント
\file{texmf-dist/doc/luatex/luatexja/luatexja-ja.pdf}を参照してください。
\end{janote}

異なるエンジンおよびそれらを用いて構築されるフォーマットの機能や歴史についての詳細は、
TUGboat記事\textsl{Overview of TeX engines and formats}（\url{https://tug.org/TUGboat/%
tb46-3/tb144berry-engines-formats.pdf}）を参照してください。また、エンジン・フォーマット・
パッケージの簡潔な比較については、以下の\url{https://tug.org/levels}を参照してください。
...
\TL をインストールする最も一般的な方法は、\TL のインターネット・インストーラを
ダウンロードすることです。より詳細な情報や\TL を入手するその他の方法については
\url{https://tug.org/texlive/acquire.html}を参照してください。
%
\begin{description}
...
\item[DVDまたはISOイメージ]
すでに展開されているはずなので、以下で説明する手順にしたがい、単に\file{%
install-tl}を実行してください。
\end{description}
%
\begin{figure}[tb]
...
macOSには\MacTeX という専用のディストリビューションがあります
（\url{https://tug.org/mactex}）。\MacTeX のインストーラはmacOS向けにいくつかの
最適化をしている（たとえば、いわゆる「{\TeX}Distデータ構造」を利用してMac上の
複数の\TL リリースを簡単に切り替える機能があります）ので、\TL インストーラを
利用するよりも、\MacTeX を利用したインストールをおすすめします\footnote{訳注：
ただしUnix向けの\TL インストーラも問題なく利用できます。}。

\MacTeX は\TL の上位集合で、主要な\TeX ツリーと含まれる実行バイナリはまったく
同一です。通常の\TL に、macOS専用のドキュメントとアプリケーションがいくつか
追加されています。
...
\end{alltt}
%
を実行してください。もちろん絶対パスを指定して起動することも可能です。
%
\begin{alltt}
> \winpath{C:\\tltemp\\install-tl-windows}
\end{alltt}
%
...
特に、Windowsのインストールに際しユーザがMicrosoftネットワークアカウントを作成することを
強制されるWindows~11で問題となり得ます。回避策については\url{https://tug.org/texlive/%
windows.html\#nonascii}を参照してください。

\begin{figure}[tb]
\begin{boxedverbatim}
Installing TeX Live 2026 from: ...
...
...
 ...
 <B> binary platforms: 1 out of 15

 <S> set installation scheme: scheme-full

 <C> customizing installation collections
     40 collections out of 41, disk space required: 9402 MB (free: 138718 MB)
...
===============================================================================
   ...
   b [ ] macOS current (11-) on ARM/x86_64 (universal-darwin)
...
で説明しました。デフォルトのインストール先はUnix系では\dir{/usr/local/texlive/2026}、
Windowsでは\winpath{\winenv{SystemDrive}\\texlive\\2026}です。この配置は複数の\TL
...
Windowsでは、通常\winpath{C:\\texlive\\2026}（より一般的に言えば\winpath{%
\winenv{SystemDrive}\\texlive\\2026}）を作成するのに管理者権限は必要ありません。
...
簡単に指定することができます（例：\dir{~/texlive/2026}）。
...
ため特に何もする必要はありません。古いオプションである\sopt{gui=perltk}と
\sopt{gui=wizard}は現在も利用可能ですが、普通のGUIインターフェースを起動します。
...
指定した\meta{言語コード}（通常2文字）にあたる言語を使用します\footnote{訳注：日本語を
使用したい場合は\sopt{lang ja}とします。}。インストーラは自動的に最適な言語で起動します
...
インストール後の処理だけを実行します。なお\file{tlpkg/texlive.tlpdb}は上書きされる
...
環境の下では\file{$HOME/.profile}（もしくは\file{.profile}から読み込まれている
別のファイルでも構いません）に次のような行を追加することで環境変数を設定できます：
%
\begin{sverbatim}
PATH=/usr/local/texlive/2026/bin/x86_64-linux:$PATH; export PATH
MANPATH=/usr/local/texlive/2026/texmf-dist/doc/man:$MANPATH; export MANPATH
INFOPATH=/usr/local/texlive/2026/texmf-dist/doc/info:$INFOPATH; export INFOPATH
\end{sverbatim}
%
...
%
\begin{sverbatim}
setenv PATH /usr/local/texlive/2026/bin/x86_64-linux:$PATH
setenv MANPATH /usr/local/texlive/2026/texmf-dist/doc/man:$MANPATH
setenv INFOPATH /usr/local/texlive/2026/texmf-dist/doc/info:$INFOPATH
\end{sverbatim}
%
利用しているプラットフォームが\code{x86\_64-linux}でない場合は、適切な
プラットフォーム名を使用してください。またデフォルトのディレクトリ以外の
場所にインストールを行った場合は、ディレクトリ名も対応するものに変更して
ください。\TL インストーラはインストールの完了時にそれぞれのフルパスを
報告します。

すでに\code{PATH}をどこかの起動時設定ファイルで設定している場合は、
...
%
\begin{sverbatim}
MANPATH_MAP /usr/local/texlive/2026/bin/x86_64-linux \
            /usr/local/texlive/2026/texmf-dist/doc/man
\end{sverbatim}
%
...
\TL をDVD（\url{https://tug.org/dvd}を参照）またはISOイメージからインストール
した後、インターネットを利用してアップデートを行いたい場合は、以下のコマンドを
（前節の説明にしたがって）検索パスを設定した\emph{後}に実行してください：
%
\begin{alltt}
> ... update PATH ...
...
通常フォント名（例えば\code{Liberation Serif}）を利用してアクセスされますが、
...
\pkg{luaotfload}パッケージのおかげです。なお\pkg{luaotfload}のフォント名
インデックスは新しいフォントについては再生成される必要がありますが、これは
未知のフォントを読み込もうとすると自動的に実行されます。

\paragraph{\XeTeX の場合}
Windowsでは\TL のフォントも自動的に利用できる状態になっているはずです
\footnote{Windows向け\TL に含まれる\prog{{fc-cache}}プログラムを実行する
ことで実現されています。}。macOSについては、別の文献を参照してください
\footnote{訳注：\TL 内のフォントを個別にインストールするだけであれば、
当該フォントの実体ファイル（例えば`\code{kpsewhich \meta{フォントファイル
名}}'を実行すれば見つけられます）をFinder上で表示してダブルクリックする
ことでFont Bookアプリが起動し、インストールすることが可能です。\TL に
含まれるインストール可能なフォントを一括で処理する場合には\dir{%
TEXMFDIST/fonts/}以下にあるフォントファイルを\dir{/Library/font}（シス
テム全体にインストールする場合）や\dir{~/Library/font}（特定ユーザのみ
にインストールする場合）にすべてコピーすればよいようです。}。macOS以外の
Unix系システムの場合の手順は以下の通りです。

\pkg{xetex}パッケージがインストールされている場合\footnote{最初から
インストールされている場合も、後から追加された場合も同様です。}、必要な
設定ファイルが\file{TEXMFSYSVAR/fonts/conf/texlive-fontconfig.conf}に
作成されています。\TL のフォントをシステムフォントとして利用可能にするため
には以下を実施します：
%
\begin{enumerate}
\item その\file{texlive-fontconfig.conf}ファイルを（典型的には）\file{%
  /etc/fonts/conf.d/09-texlive.conf}にコピー
\item \code{fc-cache -fsv}を実行
\end{enumerate}
...
\begin{enumerate}
\item その\file{texlive-fontconfig.conf}ファイルを（典型的には）\file{%
  ~/.fonts.conf.d/09-texlive.conf}（\dir{~}はホームディレクトリを表す）にコピー
\item \code{fc-cache -fv}を実行
\end{enumerate}
...
\TL のインストール後はそのままで動く状態になっているはずで、\cmd{tlmgr}を使用して
アップデートを行っている限りは特に特別な設定を行う必要はありません。

\ConTeXt は\KPS ライブラリやシステムワイドの\code{ls-R}ファイルを使用していません。
代わりに、\ConTeXt はユーザごとのファイル名データベースをすべての既知のツリーについて
管理しています。\cmd{mktexlsr}を実行してもこれらのデータベースは直接は更新されませんが、
\ConTeXt は次に実行された際に自動的にこれらのデータベースを再生成します\footnote{\TL
専用の\file{cont-sys.mkiv}と\file{cont-sys.mkxl}がその実装です。}。
...
行わない限りは、アンインストールは実行されません。（なお\lopt{all}の付かない
\code{uninstall}アクションは、1つ1つのパッケージを個別にアンインストールする
ためのものです。）
...
\end{description}
%
こうしたツリーのパス位置は\code{kpsewhich -var-value=\meta{ツリー}}を実行する
ことで確認することができます。

\prog{tlmgr}によるアンインストールではインストール後のアクション結果を復帰する
ところまでは行いません。例えばシェルの初期化ファイルにおける\code{PATH}の変更や
...
\item[\TeX 向きのエディタ] テキストエディタの選択肢はかなり多岐にわたり、その
  ...
  \item \prog{GNU Emacs}: \url{https://www.gnu.org/software/emacs/emacs.html}%
    （多くの主要なプラットフォームで利用可能）
  \item \prog{Auc\TeX}: \url{https://www.gnu.org/software/auctex}%
...
\prog{tlcockpit}で、動作にはバージョン8以降のJavaとJavaFXが必要です。これらは
いずれも独立したパッケージです。
...
fmtutil: /usr/local/texlive/2026/texmf-var/web2c/xetex/xelatex.fmt installed.
\end{fverbatim}
...
\begin{description}
\item[メニューとショートカット]
スタートメニューに`\TL'サブメニューを追加します。このサブメニューから\prog{tlshell}%
（\prog{tlmgr}のGUI）や\prog{dviout}などのGUIプログラムや、いくつかのドキュメントに
アクセスすることができます。
...
「プログラムから開く」を選択した場合に\prog{bitmap2eps}を選択できるようになります。
\prog{bitmap2eps}はシンプルなスクリプトで、バックエンドとして\prog{sam2p}や\prog{%
bmeps}を利用します。
...
\prog{tlshell}がTcl/Tkを利用することから\TL には\kenten{隠された}それらのコピーが
含まれています。これらのプログラムは\TL のプログラムからは利用可能ですが、環境変数や
レジストリ設定の上では存在していないことになっています。\TL に同梱されているPerlと
Tcl/Tkは完全版ではなく、また3プログラムともシステムに別途インストールされたものとは
一切干渉しないはずです。\TL に含まれるインフラ以外のスクリプトを、外部のPerl等で
動作させたい場合の設定方法については\ref{sec:windows-external}節を参照してください。
...
\subsection{外部のPerl, Tcl/Tk, Ghostscriptを利用する}
\label{sec:windows-external}

通常\TL は同梱のPerl, Tcl/Tk, Ghostscriptを使用します。これはインフラだけでなく、
\TL が収録するすべてのスクリプトについても同様です。ユーザ自身が別途インストール
した外部のPerl等を使用したい場合には、\emph{\TL ルート直下の}\file{texmf.cnf}で
設定を行うことができます。

外部のPerlを使用するには\file{texmf.cnf}に
%
\begin{verbatim}
TEXLIVE_WINDOWS_TRY_EXTERNAL_PERL = 1
\end{verbatim}
%
という行を追加します。こうすることで、\TL （特に\file{bin/windows/runscript.tlu}）
は\TL のインフラツール以外について検索パス内の\file{perl.exe}を探すようになります。
これはここで紹介する3つの設定のうちで最も有益なケースが多いと考えられます。\TL の
Perlも多数の追加モジュールをサポートしていますが、すべてのサードパーティスクリプトを
サポートできるほどではありません。
...
\end{verbatim}
%
という行を追加することで、\TL は検索パス内の\file{tclkit.exe}, \file{wish.exe},
\file{wish85.exe}, \file{wish86.exe}, \file{wish87.exe}を探すようになります。

Ghostscriptの場合は設定方法が異なります。この場合、使用したいGhostscriptコマンドの
ファイル名もしくはフルパスを与える必要があります。
%
\begin{alltt}
TEXLIVE_WINDOWS_EXTERNAL_GS = \meta{Ghostscriptコマンドのパス}
\end{alltt}
%
もう1つの違いは、\TL 同梱のGhostscriptは完全なものであることです。ドキュメントと
プリンタドライバのみが省略されています。したがって、ここに示した方法で\TL の設定を
上書きする必要が生じることはほとんどないでしょう。

\file{texmf.cnf}については\ref{sec:configfiles}節も参照してください。
...
\Webc システムはUnix系、Windows、macOSなどのOSで動作します。このシステムでは
...
代わりに、通常のインストールを行うと\file{.../2026/texmf.cnf}が生成されます。
どうしてもデフォルト設定を変更する必要が生じた場合は（通常は必要ありません）、この
ファイルに変更を加えてください。メインの設定ファイルは\file{.../2026/texmf-dist/%
...
その他の\Webc の便利な機能として、数々のメモリパラメタ（例えば配列サイズ）
...
\item 角藤亮。Windows用バイナリを提供し、さらに開発に関わる数々の顕著な貢献をして
  くれました。
...
Mojca Miklavec (\pkg{x86\_64-darwinlegacy}),
Norbert Preining (\pkg{aarch64-linux},
                  \pkg{armhf-linux},
                  \pkg{i386-freebsd}, \pkg{amd64-freebsd},
                  \pkg{i386-linux}, \pkg{x86\_64-linux},
...
Wang Ran、S YS、Jjgod Jiang、Jinsong Zhao、Yue Wang、Helin Gai（中国語）、
Nikola Le\v{c}i\'c（セルビア語）、
Marco Pallante、Carla Maggi（イタリア語）、
Petr Sojka、Jan Busa（チェコ・スロバキア語）、
Boris Veytsman、Vladimir Lomov（ロシア語）、
...
呼び出すプログラムについては改修の必要があります。詳細は\url{https://tug.org/%
tds/tds.html\#Scripts}を参照してください。
...
\item 従来は専用の設定ファイル\file{pdftex.cfg}で設定していたパラメタは、今後
...
\XeTeX: クロスエンジン・サポートのための新プリミティブ\cs{expanded},
\cs{creationdate}, \cs{elapsedtime}, \cs{filedump}, \cs{filemoddate},
\cs{filesize}, \cs{resettimer}, \cs{normaldeviate}, \cs{uniformdeviate},
\cs{randomseed}が追加されました。また\cs{Ucharcat}はアクティブ文字を生成
できるように拡張されました。
...
\cs{current(x)spacingmode}, \cs{ifincsname}が主にクロスエンジン・サポートのため
追加されました。また\cs{fontchar??}と\cs{iffontchar}の仕様が改訂されました。
\eupTeX に限り\cs{currentcjktoken}も利用可能になりました。
...
\pkg{x86\_64-darwinlegacy}: \cmd{xdvi}を含め、X11サポートを終了しました。
...
%
\begin{itemize}[nosep]
\item 独自の形式HINTを出力するHi\TeX が追加されました。HINTは技術文書を
  モバイル端末で読むのに適するようにデザインされたものです。HINTビューアは
  GNU/Linux, Windows, Android向けのものが開発されていますが、\TL には含まれて
  いません。
\item \prog{tangle}, \prog{weave}: 3つ目の引数（省略可）に出力先のファイルを
  指定できるようになりました。
\item \WEB プログラムについて「ミニインデックス」を作成するためのKnuth製の
  プログラム\prog{twill}が収録されました。
\end{itemize}

エンジン共通の拡張（Knuthian \TeX, Aleph, Hi\TeX を除く）：
%
\begin{itemize}[nosep]
\item \cs{show}の出力をファイルにリダイレクトする\cs{showstream}プリミティブ
  が追加されました。
\item 空行や垂直ボックスの終端などに挿入される\cs{par}トークンを別のトークンに
  変更するためのプリミティブ\cs{partokenname}, \cs{partokencontext}が追加
  されました。
\end{itemize}
...
%
\begin{itemize}[nosep]
\item 新プリミティブ\cs{lastnodefont}, \cs{suppresslongerror},
  \cs{suppressoutererror}, \cs{suppressmathparerror}が追加されました。
\item \pdfTeX の拡張\code{\cs{vadjust} pre}がサポートされました。
\end{itemize}

\LuaTeX:
%
\begin{itemize}[nosep]
\item 構造化PDF 2.0出力がサポートされました。
\item PDF 2.0のPNG /Smaskもサポートされました。
\item バリアブルフォントのインターフェースが\cmd{luahbtex}に追加されました。
\item \cs{mathdefaultsmode}のデフォルトラジカルスタイルが変更されました。
\item 選択したdiscretionaryの作成しないようにするオプション機能が追加されました。
\item TrueTypeフォント実装を改善しました。
\item \cs{fontdimen}アロケーションがより効率的になりました。
\item 方向同期ノードが後ろに続くローカル\code{par}ノードのみからなる段落は無視する
  ようになりました。
\end{itemize}

\MP: マクロ展開が停止しなくなるバグを修正しました。

\pdfTeX:
%
\begin{itemize}[nosep]
\item 構造化PDF 2.0出力がサポートされました。
\item レタースペーシング済みフォントについて、指定のある場合は明示的に
  \cs{fontdimen6}を使用するようになりました。
\item 警告は常に行頭に出力されるようになりました。
\item 自動カーニング（\cs{pdfappendkern}と\cs{pdfprependkern}）についても
  protrusionを行うようになりました。同様に自動カーニングの位置にも明示的
  もしくは暗黙的なハイフネーションが行われます。
\end{itemize}

\pTeX など：
%
\begin{itemize}[nosep]
\item 現在の\LaTeX をより強力にサポートする\pTeX\ 4.0.0にメジャー
  アップデートされました。
\item 新プリミティブ\cs{ptexlineendmode}, \cs{toucs}が追加されました。
\item これまでは\upTeX, \eupTeX でのみサポートされていた\cs{ucs}が\pTeX と\epTeX
  でもサポートされました。
\item 北川氏の\textsl{TUGboat}記事（\url{https://tug.org/TUGboat/tb41-3/%
  tb129kitagawa-char.pdf}）にあるように、8-bit文字と和文文字が区別される
  ようになりました。
\end{itemize}

\XeTeX: \XeTeX とPSTricksの透過機能（Ghostscriptの実装の問題で、この機能を
使うと安全性が損なわれます）を必要とする文書を簡単に処理できるように、新たな
ラッパ\cmd{xetex-unsafe}と\cmd{xelatex-unsafe}が追加されました。

DVIPDFM$x$:
%
\begin{itemize}[nosep]
\item \sopt{dNOSAFER}を指定しなくてもPSTricksをサポートするようになりました
  （透過機能を除く）。
\item ビットマップフォントの解像度を指定する\sopt{r}が再び動作するようになりました。
\end{itemize}

Dvips: デフォルトでは回転されたページサイズ用の自動メディア調整を試行しないように
なりました。新設の\lopt{landscaperotate}オプションを指定すると再度有効化されます。

\prog{upmendex}: 実験的にアラビア文字・ヘブライ文字をサポートしました。全般的に
文字分類と多言語サポートが強化されました。

Kpathsea: \code{kpsewhich -all}の結果において通常（非\sopt{all}）の探索の結果が
一番に出力されるようになりました。

\prog{tlmgr}とインフラ：
%
\begin{itemize}[nosep]
\item \code{mirror.ctan.org}についてデフォルトでHTTPSを使うようになりました。
\item 再配置が簡単になるように\code{SELFAUTOPARENT}の代わりに\code{TEXMFROOT}を
  使うようになりました。
\item \prog{install-tl}: あるパッケージのダウンロードやインストールに失敗した際、
 自動的に後続のパッケージのインストールを続行し、後で（一度だけ）再試行するように
 なりました。
\end{itemize}

\MacTeX: \MacTeX とそのバイナリフォルダ\dir{universal-darwin}がmacOS 10.14以上（Mojave,
Catalina, Big~Sur, Monterey）になりました。\dir{x86_64-darwinlegacy}フォルダのバイナリ
（Unix版の\prog{install-tl}でのみインストール可能）macOS 10.6（Snow Leopard）以降を
サポートします。

プラットフォーム：サポート対象のプラットフォームに増減はありません。
来年（2024年）のリリースからはWindows向けのバイナリを32-bitから64-bitに変更する
予定です。残念ながら両方を同時にうまくサポートすることはできません。

\subsubsection{2023}

Windows: 以前からアナウンスしていた通り、\TL は32-bitバイナリの代わりに64-bitバイナリを
収録するようになりました。新しいディレクトリ名は\dir{bin/windows}です（64-bitバイナリを
名前に``32''と付くディレクトリに配置するのは不適当に思われたため）。この変更がWindowsユーザ
に余計な仕事を発生させることは承知の上ですが、これよりも良い方法は見つかりませんでした。
32-bit Windowsのサポートが必要な方は、\TL 2022をインストールする必要がある場合があります。
Windows~11以降では、ARM上のWindowsは64-bit AMD/Intelをエミューレートするので最新の\TL を
利用することができます。一方、ARM上のWindows~10は64-bit AMD/Intelをエミューレートできない
ため、32-bit \TL が必要です。詳細は\TL のWindows向け特別ページ（\url{https://tug.org/%
texlive/windows.html}）も参照してください。

\TeX エンジン共通の拡張（オリジナルの\TeX と\eTeX を除く）：\cs{special}の直後に新しい
キーワード``\code{shipout}''が置かれた場合、その引数トークンの展開が\cs{shipout}時
（すなわち\cs{immediate}なしの\cs{write}と同じタイミング）まで遅延されるようになりました。

\epTeX と\eupTeX:
%
\begin{itemize}[nosep]
\item 素の(u)\pTeX はビルドされなくなりました。今後\code{ptex}, \code{uptex}コマンドは
  それぞれ\code{eptex}, \code{euptex}の互換モードを起動します。{\pTeX} toolsについても
  同様です（後述）。
\item 新プリミティブ：\cs{tojis}, \cs{ptextracingfonts}, \cs{ptexfontname}。
\item \cs{font}で新しいJIS/UCSシンタックスがサポートされました。
\end{itemize}

\LuaTeX:
%
\begin{itemize}[nosep]
\item ファミリ適用を許容したまま、数式文字のクラスを保持するための新プリミティブ
  \cs{variablefam}。
\item r2lアノテーション領域の改善。
\item エンジン共通の遅延\cs{special}（上述）。
\item {\LuaTeX} 1.17.0が5月にリリースされました：\pkg{socket}ライブラリはデフォルトでは
  無効化されました。新しいコマンドラインオプション\lopt{socket}または\lopt{shell-escape}%
  （\lopt{shell-restricted}ではありません）を利用すると\pkg{socket}が有効になります。
  また\pkg{mime}ライブラリは常に利用可能になりました。新しい関数\code{os.socketsleep}と
  \code{os.socketgettime}も常に利用可能です。その他、セキュリティの向上も図られました。
\end{itemize}

\MP: バグ修正を行いました。精度改善のため\code{svg->dx}と\code{svg->dy}は
\code{double}になりました。\code{mp\_begin\_iteration}が更新されました。
\code{mplib}のメモリリークが修正されました。

\pdfTeX:
%
\begin{itemize}[nosep]
\item \code{/Info}辞書を完全に省略するための新プリミティブ\cs{pdfomitinfodict}。
\item \code{/ProcSet}配列の省略をコントロールする新プリミティブ\cs{pdfomitprocset}：
  このパラメタの値が負のとき、または値がゼロで\pdfTeX がPDF~1.x出力を生成中の場合は
  \code{/ProcSet}が含められます。
\item \cs{pdfinterwordspaceon}下において、現在のフォントエンコーディングが
  \code{/space}文字をスロット32に持っている場合はそれが利用されます。そうでない場合は
  \code{/space}には（新規の）デフォルトフォント\code{pdftexspace}が用いられます。この
  デフォルトフォントは新プリミティブ\cs{pdfspacefont}によって上書きすることが可能です。
  この新手順は\cs{pdffakespace}でも用いられます。
\end{itemize}

\pTeX など：
%
\begin{itemize}[nosep]
\item 前述の通り、\code{ptex}は独自のバイナリではなく\code{eptex}の互換モードを
  起動するようになりました。
\item {\pTeX} tools（\prog{pbibtex}, \prog{pdvitype}, \prog{ppltotf}, \prog{ptftopl}）
  はそれぞれ\upTeX バージョンの対応ツールに統合され、互換モードで動作するようになりました。
\end{itemize}

\XeTeX: \cs{XeTeXupwardsmode}が有効になっている際の\cs{topskip}と\cs{splittopskip}の
計算に関するバグ修正と、上述のエンジン共通の遅延\cs{special}追加が行われました。

DVIPDFM$x$: pdfmark/bookmarkを作成する新オプション\lopt{pdfm-str-utf8}が追加されました。

{\BibTeX}u:
%
\begin{itemize}[nosep]
\item この\BibTeX 拡張は、オリジナルよりも遥かに強力な（Unicodeベースの）多言語
  サポートを備えた\BibTeX のほぼ完全上位互換プログラムです。\TL には数年前に
  取り込まれました。
\item 今年はCJK言語をサポートするための機能がさらに追加されました。また日本語
  組版用の\prog{(u)pbibitex}その他のプログラムと同様の拡張も行われました。
\end{itemize}

\KPS: Windows版と同様、Unix系プラットフォームでもファイルエンコーディング推定を
サポートしました。(\code{e})\code{p}(\code{la})\code{tex}と\code{pbibtex}で
有効になっています。

\prog{tlmgr}とインフラ：
%
\begin{itemize}[nosep]
\item macOSでのデフォルトをテキストモードに変更しました。
\item コアパッケージを先にインストールし、それ以外のパッケージのリトライは1回に
  なりました。
\item ディスクの空き容量が十分かどうか、簡易チェックを行うようになりました。
\end{itemize}

\MacTeX:
%
\begin{itemize}[nosep]
\item \MacTeX とそのバイナリディレクトリ\dir{universal-darwin}の要求は
  macOS 10.14以上になりました（Mojave, Catalina, Big~Sur, Monterey, Ventura）。
  Unix版の\prog{install-tl}でのみ利用可能な\dir{x86_64-darwinlegacy}バイナリ
  ディレクトリはmacOS 10.6（Snow~Leopard）以降をサポートしています。
\item \MacTeX のGUIパッケージに、macOS向けのHINT文書\footnote{\prog{hitex}や
  \prog{hilatex}エンジンで作成される、モバイル向け文書。詳細はHi\TeX の
  ウェブサイト（\url{https://hint.userweb.mwn.de/hint/hitex.html}）を参照して
  ください。}ビューア\prog{hintview}が追加されました。GUIパッケージはドキュメント
  フォルダはインストールしなくなりました。代わりに、新規ユーザのための短い
  \file{READ ME}と\prog{hintview}に関するページが追加されました。
\item DVDの\dir{Extra}ディレクトリ内の追加\TeX ソフトウェアは、ダウンロード
  サイトのリンクを含むドキュメントに置き換えられました。
\end{itemize}

プラットフォーム：
%
\begin{itemize}[nosep]
\item 前述の通り、新しい\dir{windows}バイナリディレクトリは64-bit Windowsバイナリを
  格納しています。
\item \dir{bin/win32}バイナリディレクトリは削除されました。32-bitと64-bitの両方を
  同時にサポートすることはできないためです。
\item \dir{i386-cygwin}バイナリディレクトリも削除されました。Cygwinがi386のサポートを
  終了したためです。
\end{itemize}
...
\item 内部ループのわずかな最適化（いずれの変更もDavid Fuchs由来）。
\end{itemize}
...
このライブラリは\lopt{numbersystem=interval}で有効化されます。

\pdfTeX:
%
\begin{itemize}[nosep]
\item ビッグエンディアンシステムにおけるテキスト抽出の信頼性向上。
\item 破損したPFB, FMT, PDFファイルに対するチェックの強化。
\item 異常なPDFのインクルードがあった場合に、破損したPDFを生成するのを抑止。
\item マニュアルを\LaTeX に変換し、校正も実施。ただし重要な更新はわずかです。
\end{itemize}

\epTeX, \eupTeX:
%
\begin{itemize}[nosep]
\item 新しいシンプルなエンコーディング変換ユーティリティ\cmd{ptekf}。
\item \cmd{eptex}の独立バイナリを廃止。このコマンドは、今後は\cmd{euptex}のレガシー
...
%
\begin{itemize}[nosep]
\item \pdfTeX と同じ方法でタイムゾーンオフセットを計算。
\item 新しいAdobe-Japan1-UCS2 CMapに対応するために異体字セレクタを処理。
...
%
\begin{itemize}[nosep]
\item Lua\LaTeXTeX に向けて、\code{TEXMF}(\code{SYS})\code{VAR}以下への書き出しも許可するべく
  ファイル名の拡張された安全性チェックをサポート。これにともない、新しい関数と対応する
  \cmd{kpsewhich}のオプションを追加。
...
%
\begin{itemize}[nosep]
\item バックエンド\TeX エンジンとして\LuaTeX をサポート。
\item ブレークポイントのない行におけるバッファオーバーフローを修正。
\end{itemize}

\prog{tlmgr}:
%
\begin{itemize}[nosep]
\item バグ報告情報にアクセスするための新アクション\code{tlmgr bug}。
\item LWPサポートを強化するため（特にmacOSで重要）、独自のルート証明書バンドルをすべての
  プラットフォームで利用。
\item ローカルの\TL より古いリモートリポジトリ（つまりミラーの状態）を検出した場合は
  アップデートを行わないことで、誤ったパッケージ削除を防止。
\item \ConTeXt の紙面サイズ設定をサポート。
\item ミラーリポジトリ（mainとtlcontrib）用のtlpdbsakeバックアップを除去。
\end{itemize}

\MacTeX: 最新版のGhostscript 10.02.1を同梱。4つのGUIプログラムと最新\TL を収録。バイナリは
macOS 10.14（Mojave, 2018）以降のサポートを維持（いつも通り、古いMacは\TL に含まれる
\pkg{x86\_64-darwinlegacy}バイナリではサポートされていますが、\MacTeX ではサポート外です）。

\subsubsection{2025}

エンジン共通の拡張（Knuthian \TeX を除く）：
%
\begin{itemize}[nosep]
\item すべてのフォーマット（plain \TeX と\LaTeX の両方）でPDF-1.7を生成するように変更。
  以前は1.5でした。印刷所などのために、古いバージョンを出力したい場合があり得ます。
  そのような場合はいくつか方法があり、例えばPDF-1.4を出力したい場合は次のようにします：
  \begin{itemize}[nosep]
  \item \cmd{ps2pdf14}を\TeX が生成したPDFに対して実行します。このシンプルなラッパーは
    Ghostscriptに含まれています。それ以外の外部ソフトウェアも同様の処理が可能です。
  \item \LaTeX のPDFバックエンドを用いる場合：\verb|pdf_version_gset:n{1.4}|
  \item \pdfTeX: \verb|\pdfmajorversion=1 \pdfminorversion=4|
  \item \LuaTeX: \verb|\pdfvariable majorversion=1 \pdfvariable minorversion=4|
  \item \XeTeX: \verb|\special{pdf:majorversion:1}\special{pdf:minorversion:4}|
  \end{itemize}
\item \TeX が\sopt{halt-on-error}オプションが原因で終了した場合、ヘルプメッセージを
  ログファイルに書き出します。これはデフォルトの挙動と一致します。
\item 新しいプリミティブパラメータ\cs{ignoreprimitiveerror}：1に設定された場合、
  ``\code{Infinite glue shrinkage found in box being split}''のエラーは警告に
  下げられ、プログラムの終了ステータスが失敗に変更されません。それ以外の数値は
  将来的な変更のために予約されています（背景は\href{https://www.latex-project.org/%
  news/latex2e-news/ltnews39.pdf}{{\LaTeX} News 39}の``Enhancements to the new
  mark mechanism''セクション）。
\item フォントを2048\,pt以上に設定した際にはエラーメッセージを出力します。以前は
  （手当されることのない）算術オーバーフローまたはユーザの指定値の暗黙的変更が
  行われていました。
\end{itemize}
...
\item 新しい\code{luaffi}ライブラリ、aarch64のサポート。
\item \cs{tracinglostchars=4}指定時には、組版時に必要になるのを待たずにグリフ
  処理時に欠落文字について警告を出します。\cs{tracinglostchars=5}ではこの
  \kenten{早めの}診断情報をエラーにします。
\item 新しいプリミティブ\cs{mathemptydisplaymode}：1に設定された場合、
  ディスプレイ数式の前の空の段落は無視されます。デフォルトでは、ディスプレイ数式
  前のdirノードを含む段落は無視されません。これは伝統的な挙動と一致します。
\item PNG処理時、今後はマスクにattrは設定されません。この削除変更について要望を
  受けた場合には\code{makeattr}機能を追加することは可能ですが、そのような場合は
  画像ではなく適切なPDFを埋め込むことを推奨します（その方が堅牢です）。
\item 即時代入時にトークン結合子を処理。
\item 先述のエンジン共通変更。
\end{itemize}
...
%
\begin{itemize}[nosep]
\item 新しい\cs{kcatcode}（\code{modifier}: 20, \code{latin\_ucs}: 14）により
  複数のコードポイントをもつ結合文字、Unicodeラテン文字および16-bit OFM
  Level-0をサポート。
\item 先述のエンジン共通変更。
\end{itemize}
...
%
\begin{itemize}[nosep]
\item \cmd{extractbb}実行ファイルは\prog{dvipdfmx}へのリンクではなく、独立に
  メンテナンスされるスクリプトとなりました。
\item 仮想フォントを介した文字結合のため\upTeX の新しいエンコーディングをサポート。
\item 127個以上のグリフを持つTrueTypeフォント埋め込みの修正。
\item 画像を複数回読み込む際の問題の修正。
\end{itemize}

CWEB:
%
\begin{itemize}[nosep]
\item \prog{cweave}と\prog{ctwill}に混合ステートメントのための新しい\code{+F}%
  オプションを追加。
\item PDFのブックマークページとtable-of-contentsを同期。
\item Martin Ruckert氏の手法によるインデックスソートの高速化。
\item names-of-the-sectionsをソートするための新しいマクロ\cs{9}。
\item \prog{ctwill-proofsort}のソートアルゴリズムを改善。
\item \prog{ctwill}フォーマットでnames-of-the-sectionsを出力。
\end{itemize}
...
%
\begin{itemize}[nosep]
\item \cmd{tlmgr info}：与えられたパッケージのもつファイルを報告するだけの
  新オプション\lopt{only-files}。
\item \cmd{tlmgr search}：新オプション\lopt{json}。
\end{itemize}

プラットフォーム：
%
\begin{itemize}[nosep]
\item CentOS 7のサポートが2024年7月30日に（セキュリティアップデートも含めて）終了
  したことにともない、\pkg{x86\_64-linux}バイナリはAlma Linux 8でビルドされるように
  なりました。我々はこのことを残念に思っていますが、他の代替が見つかりませんでした。
  古いシステム向けのバイナリは利用可能です（\url{https://tug.org/texlive/custom-%
  bin.html}を参照）。
\item \TL は\file{scripts-targets.lst}ファイルの提供を開始します。このファイルは
  シンボリックリンクの貼られたスクリプトのリンク名とターゲットを含んでおり、
  ディストリビューション作成者の利便性が念頭にあります。このファイルは\TL の
  ソースディレクトリ\dir{Build/source/texk/texlive/linked_scripts}にあります。
  同ディレクトリに以前より長く存在するファイル\file{scripts.lst}のメンテナンスは
  継続されます。
\end{itemize}

Windows: \prog{uninstall-windows}や\prog{runscript}で使用されていたVisual Basicは
Powershellに置き換えられました。これはVBはWindows~11ではオプショナルであるためです。

\MacTeX: 今年も\MacTeX-2025はmacOS 10.14（Mojave, 2018）以上でArmとIntelの両方の
プロセッサをサポートします。これによってインストールされるものは以下の通りです。
%
\begin{itemize}[nosep]
\item {\TL} 2025。
\item Ghostscript-10.04.0。
\item \pkg{dvisvgm}の\pkg{libgs}と\pkg{mutool}。従来は\pkg{dvisvgm}サポートはX11を
  必要としていましたが、2025年にこの要件は削除され、\pkg{dvisvgm}はX11の有無に
  かかわらず動作するようになりました。
\item HintView 2.0：最新のHi\TeX に追加された色拡張にも対応。
\item Asymptote：macOS 10.11（Big Sur, 2020）以上が必要。
\item {\TeX}Shop 5.49：作文ツールなど最新のApple Intelligence機能をサポート。
\end{itemize}

\subsection{現在：2026}
\label{sec:tlcurrent}

エンジン共通の変更（Knuthian \TeX とAlephを除く）：
``\code{Infinite glue shrinkage}''のエラーが無視されている場合（2025年のリリース
ノートを参照）には``error''という語がメッセージに含まれないようになり、またその
メッセージはログファイルのみに出力されるようになりました。以前の挙動は軽微な混乱や
誤検出の原因となっていました。

Hi\TeX:
%
\begin{itemize}[nosep]
\item 入力エンコーディングはUTF-8のみを使用するようになりました。
\item OpenTypeとTrueTypeフォントの最小限のサポートを追加。リガチャやmathの
  サポートはありませんが、これはいずれ改善されます。
\item このバージョン向けの\LaTeX カーネルの更新が必要です。そのため、現時点では
  \cmd{hilatex}は利用できません。
\item プリミティブ\cs{Uchar}と\cs{Ucharcat}の追加。
\item 上記の実装のため、HINTビューアを含め、コードベース全体にわたる大規模な
  変更が行われました。注意して使用してください。
\item 先述のエンジン共通変更。
\end{itemize}
...
%
\begin{itemize}[nosep]
\item 新しい\code{luaffi}。
\item HarfBuzzのSubset API用のLuaバインディング。
\item \code{read\_img}が指定された\lopt{output-directory}を検索するようになりました。
\item \code{pdf->objcompresslevel}を\code{pdf->compress\_level}とともに保存および
  復元します。
\item \code{texlua}のエラーはトレースバックを表示します。
\item \code{set\_macro}における末尾スペース除去チェックが改善されました。
\item \cs{matheqdirmode=1}使用時のディスプレイ周辺の不正なスペーシングを部分的に
  修正しました。
\item \cs{mathflattenmode}を復元しました。
\item TrueTypeフォントの旧式の処理に関する修正。
\item Type 1フォント向けの\code{ToUnicode}の実験的拡張。
\item PDF出力におけるdestination namesテーブルの最大サイズが1048576に引き上げられました。
\item writeノード（\code{getfield}）に、未展開トークン用の\code{value}フィールドが
  追加されました。
\item 一部の使用されていないモードを廃止。
\item 先述のエンジン共通変更。
\end{itemize}
...
%
\begin{itemize}[nosep]
\item \code{mp\_number\_to\_scaled}は、システム依存の\code{int}ではなく、64ビット符号
  付き整数を返すようになりました。内部的にはより一般に\code{int}を\code{int64\_t}に
  置き換える作業を開始しています。
\item \code{mp\_hard\_times}を修正。
\item \cmd{weave}に関する修正。
\item ヘルプメッセージを再編成し、一貫性のため更新しました。
\end{itemize}

\pdfTeX:
%
\begin{itemize}[nosep]
\item PDFのメジャーバージョンが1の場合を除き、マイナーバージョンが2以下であることに
  ついて警告しなくなりました。
\item \cs{pdfpageheight}が使用された場合に、分数の分母やその他のテキストを誤って
  シフトしないよう修正しました。
\item \code{ptmr8r}およびその他のフォントで発生していたテキスト重なりを修正しました。
\item 環境変数\var{TEXMF\_DEBUG\_PNG\_COPY}が1に設定されている場合、各PNG画像の属性と
  それがコピーされたものかどうかをレポートします。
\item 先述のエンジン共通変更。
\end{itemize}

\epTeX, \eupTeX: 先述のエンジン共通変更。

\XeTeX: 先述のエンジン共通変更。

DVIPDFM$x$: 出力を標準出力（stdout）に送るためのオプション\sopt{o -}を再有効化
しました。これは数年間、意図せず無効化されていました。

CWEB:
%
\begin{itemize}[nosep]
\item すべてのCWEBプログラムが再びデフォルトで\lopt{verbose}に振る舞います。
\item すべてのCWEBプログラムが警告メッセージではエラー終了しなくなりました。
\item \cmd{ctangle}がプリプロセッサマクロ内の空白を大幅に削減するようになりました。
\item \cmd{ctwill-proofsort}が同一識別子に対する複数定義をサポートしました。
\end{itemize}

\prog{upmendex}: ブラーフミー系文字（ベンガル語、グルムキー語、グジャラート語、
カンナダ語、ラオ語、マラヤーラム語、オリヤー語、シンハラ語、タミル語、テルグ語）
への実験的サポート。

Kpathsea:
%
\begin{itemize}[nosep]
\item 新しい設定値\code{texmf\_nlink\_for\_leaf}を追加しました。これは、leaf%
ディレクトリにいるかどうかを判定するために\code{st\_nlink=2}チェックを行うかどうかを
指定します。デフォルトの\file{texmf.cnf}では、このチェックは現在無効化されています。
\item 設定値\code{openin\_any}はもはや効果を持ちません。インターフェース関数は引き続き
ライブラリにコンパイルされていますが、現在はどのファイル名も読み取り可能とみなします。
把握されている限りでは、`\code{a}'以外の値が有用と（あるいは使用）されたことはありません。
\end{itemize}

\prog{tlmgr}:
%
\begin{itemize}[nosep]
\item バックアップコンテナ作成失敗をチェックするようになりました。
\item \lopt{usermode}において、\pkg{texlive-scripts}パッケージが存在しないことに
ついて警告しなくなりました。
\end{itemize}
...
%
\begin{itemize}[nosep]
\item \prog{fmtutil}, \prog{updmap}: \prog{tlmgr}と同様に自己位置検出（self-locating）
  を行うようになりました。
\item 新しいプログラム\prog{xdvipsk}。これは、より多くの画像形式および（LuaTeX使用時）
  OpenTypeおよびTrueTypeフォントをサポートする\prog{dvips}拡張です。詳細はSigitas
  TolusisによるTUGboat記事（\url{https://tug.org/TUGboat/tb38–2/tb119tolusis.pdf}）を
  参照してください。
\item 多くのmanページが大幅に更新されました。
\end{itemize}

プラットフォーム：
%
\begin{itemize}[nosep]
\item 後述の\MacTeX のノートを参照してください。
\item \pkg{i386-linux}, \pkg{aarch64-linux}, \pkg{armhf-linux}のバイナリは現在
  Debian bullseye（初回リリース 2021年）上でビルドされています。
\end{itemize}

\MacTeX:
%
MacTeX-2026はBig Sur（2020年、macOS 11）以降を、IntelおよびArmの両方でサポートします。
Mojave（2018年、10.14）および Catalina（2019年、macOS 10.15）のサポートは終了しました。
インストールされる内容は以下のとおりです。
%
\begin{itemize}[nosep]
\item \TL\ 2026
\item Ghostscript-10.06.0
\item \TeX Shop（Liquid Glass 対応の特別バージョンを含む）
\item LaTeXiT, BibDesk, HintView
\end{itemize}

macOS向けの小型版Basic\TeX ディストリビューション（ダウンロードサイズ約120MB、
\url{https://tug.org/mactex/morepackages.html}）には、現在タグ付けサポートのための
すべてのコアパッケージ、テスト用の\prog{latex-dev}エンジン、および\prog{tex4ht}が
含まれています。他のプラットフォームにこれをインストールしたい場合は\pkg{scheme-small}%
を使用できます。Basic\TeX は3つの主要な組版エンジン\pdfLaTeX, \XeLaTeX, \LuaLaTeX を
すべてサポートします。

最後に、長年にわたり\TL 、とりわけWindowsサポートに尽力してくださったSiep
Kroonenberg氏に心より感謝いたします。彼女は引退し、他のプロジェクトへと進まれます。
あらためて感謝するとともに、今後のご活躍をお祈りします。
...
\begin{samepage}
間違いの指摘や提案、ヘルプの要望は下記にご連絡ください。メーリングリストを
購読せずに投稿しても構いませんが、英語で書いてください。
%
\begin{quote}
